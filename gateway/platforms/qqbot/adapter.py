...
class QQAdapter(BasePlatformAdapter):
    ...
    async def _handle_c2c_message(
        ...
        ocr_info = att_result.get("ocr_info", "")
    ...
    async def _handle_group_message(
        ...
        _safe_d = {k: (str(v)[:500] if not isinstance(v, (str, int, float, bool, list, dict, type(None))) else v) for k, v in d.items()}
        logger.info(
            "[%s] GROUP_AT_MESSAGE_CREATE 完整事件: %s",
            self._log_tag,
            _json.dumps(_safe_d, ensure_ascii=False, default=str),
        )
    ...
    # ------------------------------------------------------------------
    # Quoted-message handling (对齐 openclaw-qqbot 格式)
    # ------------------------------------------------------------------
    ...
    async def _process_quoted_context(
        ...
        对齐 openclaw-qqbot 格式：
        - 发送者标签: senderName (senderId)
        - 附件标签: MEDIA:path / [图片] / [语音消息（内容: "..."）] / [视频] / [文件]
        - 表情解析: <faceType=...> → 【表情: ...】
        - 引用索引缓存: 通过 message_scene.ext 建立 REFIDX 映射
        """
        empty = {
            "quote_block": "",
            "image_urls": [],
        ...
        for elem in elements:
            if not isinstance(elem, dict):
                continue
            # 发送者信息
            elem_author = elem.get("author") if isinstance(elem.get("author"), dict) else {}
            sender_name = elem_author.get("username", "")
            sender_id = elem_author.get("id", "")
            is_bot = elem_author.get("bot", False)

            # 发送者标签
            sender_label = f"{sender_name} ({sender_id})" if sender_name and sender_id else (sender_name or sender_id or "未知")

            # 消息内容（解析表情）
            etext = str(elem.get("content", "")).strip()
            if etext:
                etext = self._parse_qq_face_tags(etext)
                quote_parts.append(f"[{sender_label}]: {etext}")
            else:
                quote_parts.append(f"[{sender_label}]")
        ...
        # 附件标签（对齐 openclaw-qqbot formatAttachmentTags）
        attachment_tags = self._format_attachment_tags(
            all_attachments, quoted_voice, quoted_images
        )
        if attachment_tags:
            quote_parts.append(attachment_tags)

        if not quote_parts and not quoted_images:
            return empty
    ...
    @staticmethod
    def _parse_qq_face_tags(text: str) -> str:
        """解析 QQ 表情标签 <faceType=...> → 【表情: NNN】"""
        import re
        def _replace_face(m):
            face_id = m.group(1) or "?"
            return f"【表情:{face_id}】"
        return re.sub(r'<faceType=(\d+)[^>]*>', _replace_face, text)

    @staticmethod
    def _format_attachment_tags(
        attachments: List[Dict[str, Any]],
        voice_transcripts: List[str],
        image_urls: List[str],
    ) -> str:
        """格式化附件标签，对齐 openclaw-qqbot 的 formatAttachmentTags。

        格式：
        - 有路径: MEDIA:/path  或  MEDIA:/path（内容: "transcript"）
        - 无路径: [图片] / [语音消息（内容: "..."）] / [视频] / [文件: name]
        """
        if not attachments:
            return ""
        ...
        for att in attachments:
            ...
            if ct.startswith("image/"):
                if img_idx < len(image_urls):
                    parts.append(f"MEDIA:{image_urls[img_idx]}")
                    img_idx += 1
                else:
                    parts.append(f"[图片{f': {filename}' if filename else ''}]")

            elif "voice" in ct or "audio" in ct or "silk" in ct or "amr" in ct:
                transcript = voice_transcripts[voice_idx] if voice_idx < len(voice_transcripts) else ""
                voice_idx += 1
                if transcript:
                    parts.append(f'[语音消息（内容: "{transcript}"）]')
                else:
                    parts.append("[语音消息]")

            elif ct.startswith("video/"):
                parts.append(f"[视频{f': {filename}' if filename else ''}]")

            elif ct.startswith("application/") or ct.startswith("text/"):
                ...
                if url:
                    ...
                    parts.append(f"[附件{f': {filename}' if filename else ''}]")

        return " ".join(parts)

    @staticmethod
    def _merge_quote_into(text: str, quote_block: str) -> str:
        """将引用块前置到文本前，格式对齐 openclaw-qqbot。"""
        if not quote_block:
            return text
        if text.strip():
            ...
            if self._is_voice_content_type(ct, filename):
                ...
                try:
                    ...
                    if cached_path and os.path.isfile(cached_path):
                        ...
                        ocr_text = self._ocr_image(cached_path)
                        if ocr_text:
                            image_urls[-1] = cached_path  # already added
                            ocr_texts.append(f"[图片OCR] {ocr_text}")
                    elif cached_path:
                        logger.warning(
    ...
    def _ocr_image(image_path: str) -> str:
        """Run Tesseract OCR on an image, returning detected text.
        
        Returns empty string if OCR fails or no text is found.
        Skips images larger than 8MB or smaller than 5KB to avoid wasting time.
        """
        try:
            size = os.path.getsize(image_path)
            if size > 8 * 1024 * 1024 or size < 5 * 1024:
                return ""
            import pytesseract
    ...
    async def _send_group_text(
        ...
        :param keyboard: Optional inline keyboard attached to the message.
        """
        # ── MEDIA: tag interceptor ────────────────────────────
        # Extract and send MEDIA: paths as native QQ images/files
        # before delivering the cleaned text.  Mirrors the
        # openclaw-qqbot <qqimg> pattern.
        media_paths = self._MEDIA_TAG_RE.findall(content)
        if media_paths:
            import os as _os
            cleaned = self._MEDIA_TAG_RE.sub("", content).strip()
            cleaned = re.sub(r'\n{3,}', '\n\n', cleaned)
            # Send media first (images appear before text on QQ)
            for mpath in media_paths:
                expanded = _os.path.expanduser(mpath)
                if _os.path.isfile(expanded):
                    ext = _os.path.splitext(expanded)[1].lower()
                    if ext in {'.jpg', '.jpeg', '.png', '.gif', '.webp'}:
                        await self.send_image_file(group_openid, expanded, reply_to=reply_to)
                    else:
                        await self.send_document(group_openid, expanded, reply_to=reply_to)
            if not cleaned:
                return SendResult(success=True, message_id="media_only")
            content = cleaned
        # ── end interceptor ─────────────────────────────────────
